"""FreeRouting interface: writes Specctra DSN files, runs freerouting.jar,
and parses SES output into routed paths (requires Java 17+)."""
...
def write_dsn(
    ...
    """Write a Specctra .dsn file; trace i connects starting point i to test point i."""
    ...
    # Each net connects start_i to tp_i
    a('  (network')
    for i in range(n):
        a(f'    (net "net_{i}"')
...
def run_freerouting(
    ...
    """Run FreeRouting on a DSN file; returns True if an SES file was produced."""
    if jar_path is None:
...
def parse_ses(
    ...
    """Parse a Specctra SES file; returns (paths, lengths, failures) like route_all_traces."""
    ...
    # Matches: (net net_N (wire (path F.Cu width x1 y1 x2 y2 ...)))
...
def route_with_freerouting(
    ...
    """Write DSN, run FreeRouting, parse SES; drop-in for routing.route_all_traces()."""
    n = min(len(board.traces), len(test_points))
    if n == 0:
        return [], [], 0

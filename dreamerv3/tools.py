...
# numpy 2.x compat: numpy>=2.1 removed the `newshape` kwarg of np.reshape, but the
# tensorboard video writer bundled with torch still calls np.reshape(V, newshape=...)
# (in torch/utils/tensorboard/_utils.py:_prepare_video). Without this shim, video
# logging raises TypeError and crashes training at the first eval. Thin pass-through
# that accepts either `shape` or the legacy `newshape`.
if not getattr(np.reshape, "_newshape_compat", False):
    _np_reshape = np.reshape

    def _reshape_compat(a, shape=None, *args, newshape=None, **kwargs):
        return _np_reshape(a, shape if shape is not None else newshape, *args, **kwargs)

    _reshape_compat._newshape_compat = True
...
class Logger:
    ...
    def write(self, fps=False, step=False):
        ...
        for name, value in self._videos.items():
            ...
            except Exception as e:  # telemetry must never crash a long training run
                print(f"[logger] skipped video '{name}': {e}")
